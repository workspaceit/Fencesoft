...
<div id="content" class="row">
				...
				<form name="signup-form" id="signup-form" action="<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=users/registration" method="post">
				 ...
				 <form name="login-form" id="login-form" action="<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=users/login" method="post">
                        <div id="login-fields" class="drawer">
                            ...
                            <ul id="LoginWidget">
                                <li class="field"><input name="LoginForm[username]" id="username" placeholder="User Name" required="required" type="text" class="xwide text" value="" /></li>
                                <li class="field append"><input name="LoginForm[password]" id ="pass" placeholder="Password" required="required" type="password" class="normal text" value="" style="width:66%;" />
	...
	<div class="eight columns">
		<div id="drawing-tool" class="widget-box widget">
			...
			<div id="toolset-btns">						<div style="float:left;">
				<div class="medium btn primary pretty icon-right icon-cog widget-btn"><button id="GenerateQuote" name="generate_quote">Generate Quote<i class="icon-cog"></i></button></div>
				<div class="medium btn info pretty icon-right icon-folder widget-btn"><button id="SaveChanges" name="save_quote">Save Changes<i class="icon-folder"></i></button></div>
				<div class="medium btn warning pretty icon-right icon-trash widget-btn"><button id="ClearData" name="clear_data">Clear Data<i class="icon-trash"></i></button></div>
            </div>			<div style="float:left; margin:15px 0 0 30px;">			
                 <div style="float:left; margin:5px 10px 0 0;">
                    <label>
                       <input name="activefence" value="Fence Removal" type="checkbox" id="activefence" /> Fence Removal
                   </label>
                </div>
                <div style="float:left;">                    <select style="width:90px; height:30px; margin-right:10px;" id="material_type" class="select2" title="Choose your material" disabled="disabled">
                        <option></option>
                        <option value="aluminum" data-removal="<?php echo $storeData['removal_aluminum'];?>" <?php if(isset($quotesInfo['removal_fence'])){if($quotesInfo['removal_fence']=='aluminum'){ ?>selected="selected"<?php } }?>>Aluminum</option>
                        <option value="chainlink" data-removal="<?php echo $storeData['removal_chainlink'];?>" <?php if(isset($quotesInfo['removal_fence'])){if($quotesInfo['removal_fence']=='chainlink'){ ?>selected="selected"<?php } }?>>Chainlink</option>
                        <option value="vinyl" data-removal="<?php echo $storeData['removal_vinyl'];?>" <?php if(isset($quotesInfo['removal_fence'])){if($quotesInfo['removal_fence']=='vinyl'){ ?>selected="selected"<?php } }?>>Vinyl</option>
                        <option value="wood" data-removal="<?php echo $storeData['removal_wood'];?>" <?php if(isset($quotesInfo['removal_fence'])){if($quotesInfo['removal_fence']=='wood'){ ?>selected="selected"<?php } }?>>Wood</option>
                    </select>
                </div>
                <div  style="float:left;" >
                    <input id="feetRemoval" style="width:60px; margin-right:10px;" type="text" value="<?php if(isset($quotesInfo->feet_removal)){ echo $quotesInfo->feet_removal; }?>" class="form-control" disabled="disabled"/>FT
                </div>						</div>
			</div>
            <input type="hidden" id="notestext" value="<?php if(isset($quotesInfo->notes)){ if($quotesInfo->notes){echo $quotesInfo->notes; }}?>" />
            <div class="col-md-10"><textarea rows="6" name="notes" id="notes" class="form-control wysiwyg" style="height:110px; width:100%; margin:5px 0 5px 0;" placeholder="Installation Notes"></textarea></div>
            </div>
			<div class="hide">
			    <form id="pdf_generate_form" method="post" name="pdf_generate_form" action="<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=index/generateQuotes">
                    ...
                    <input type="hidden" name="quotes_line_post" value="">
...
$(function(){
	...
	$('button[name=generate_quote]').on('click',function(){
		...
		var corner_post = 0, end_post = 0,blank_post = 0,end_pos_gate = 0, line_post = 0,
		...
		$.map(drawingPoints,function(e,i){
			...
			if(lines.length > 1){
				var angleInDegree = getAngleFromTwoLines(lines[0], lines[1]);
                if(angleInDegree < 45.0) {
                    line_post++;
                }
				if(!isEnd && angleInDegree >= 45.0){
					corner_post++;
				}
		...
		stage.toDataURL({
		    callback: function(dataUrl) {			    
                ...
                $('input[name=quotes_line_post]').val(line_post);
