...
<table style="width:100%;">
	...
	<?php if(!empty($selectedMaterial['end_post_specs']) && ($total_end_post!=0)):?>
	<tr>
		<td style="height:16px;"><span style="font-size:12px;"><?php echo $total_end_post;?></span></td>
		<td style="height:16px;"><span style="font-size:12px;"><?php echo $selectedMaterial['end_post_item'];?> <?php echo $selectedMaterial['end_post_sku'];?></span></td>
		<td style="height:16px;"><?php if(($storeData['show_item_price']==1) && ($selectedMaterial['pricing'] != 'Price-per-foot')) {?><span style="font-size:12px;">$<?php echo number_format(trim($selectedMaterial['end_post_price'], "$"), 2, '.', ',');?></span><?php } ?></td>
		<td style="height:16px;"><span style="font-size:12px;"><?php echo $selectedMaterial['end_post_specs'];?></span></td>
		<td style="height:16px;">
		    <span style="font-size:12px;">
		        <?php $total = $total_end_post * number_format(trim($selectedMaterial['end_post_price'], "$"), 2, '.', ',');
                if(($storeData['show_item_price']==1) && ($selectedMaterial['pricing'] != 'Price-per-foot')) { echo '$'.number_format($total, 2, '.', ','); }
		        $totalCost +=$total;
		        ?>
	        </span>
	...
	if ($storeData['multiplier_site'] == 1) {
		$totalCost = (($totalCost - $totalConcrete) * ($storeData['multiplier_vinyl'] / 100)) + $totalConcrete;
	}
	...
	if($storeData['is_labor']=='on'){
		//check is labor cost set in material level
		if($selectedMaterial['labor_cost']!=''){
			...
			$total_labor_cost = $storeData['vinyl_labor_cost'] * ($total_length + $total_gate_width);
		}
	...
	$storeData['markup_vinyl'] = (!empty($storeData['markup_vinyl'])) ? $storeData['markup_vinyl'] : 0;
	...
	if($storeData['sales_tax_method']=='markup_materials'){
		...
		if($storeData['markup_method']=='markup_of_cost'){
			$markup_total = $totalCost * ($storeData['markup_vinyl']/100);
		...
		// Markup method for markup of selling price
			...
			$markup_total = $totalCost / (1-($storeData['markup_vinyl'])/100);
